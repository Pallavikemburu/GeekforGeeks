...
class Solution
{
    ...
    void treetrav(Node *root,unordered_map<Node*,Node*>&mp,unordered_set<Node*>&st,int k)
    {
        if(root==NULL)return;
        if(root->left)
        {
            mp[root->left]=root;
            treetrav(root->left,mp,st,k);
        }
        if(root->right)
        {
            mp[root->right]=root;
            treetrav(root->right,mp,st,k);
        }
